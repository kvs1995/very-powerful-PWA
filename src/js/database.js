...
const initdb = async () =>
  openDB('ktate', 1, {
    upgrade(db) {
      if (db.objectStoreNames.contains('ktate')) {
        console.log('ktate database already exists');
        return;
      }
      db.createObjectStore('ktate', { keyPath: 'id', autoIncrement: true });
      console.log('ktate database created');
    },
  });
// Method that takes some content and adds it to the IndexedDB database using the idb module
export const putDb = async (content) => {
  console.log('PUT to the database');
  const ktateDb = await openDB('ktate', 1);
  const tx = ktateDb.transaction('ktate', 'readwrite');
  const store = tx.objectStore('ktate');
...
export const getDb = async () => {
  ...
  const ktateDb = await openDB('ktate', 1);
  const tx = ktateDb.transaction('ktate', 'readonly');
  const store = tx.objectStore('ktate');
